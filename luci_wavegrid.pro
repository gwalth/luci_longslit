Pro luci_wavegrid,exposure,slitgrid,wavegrid,slitrange=slitrange,$
                     list=list,outlist=outlist

;+-------------------------------------------------------------------------
;
; LUCI_WAVEGRID  2/2009 revised from NIRSPEC_WAVEGRID         4/2005
;
;
...
;      Revised 2/2009    Fuyan Bian @Steward Observatory
;--------------------------------------------------------------------------
...
;   cut = median(obj(698:702,*),dim=1)
   cut = median(obj(*,1398:1402),dim=2)
;   help, cut
;   y   = findgen(1024)
   x = findgen(2048)
   ...
   !p.multi = [0,1,1]
   plot,x,cut,/xstyle,title='Select lines to trace',xtitle='x'
   print,'Left-click on lines to trace.  Lines should be fairly well spaced.'
   print,'Right-click to exit.'
   getpoints,linex,linecounts,/plotx
   nlines = n_elements(linex)
   liney  = 1400 + fltarr(nlines)
endelse
...
if keyword_set(slitrange) then range=slitrange else range=[560,1470]

;;; Trace lines
chan,11
traceline,obj,linex,liney,xtr,ytr,slittr,/ydir,nside=10,/bygauss,$
...
for i=0,nlines-1 do begin
   ...
   while not(fit_converged) do begin
      temp_b = poly_fit(temp_y(good_ls),temp_x(good_ls),trace_order,$
                        yerror=xerr)
      temp_xfit   = 0*temp_x
      for j=0,trace_order do temp_xfit = temp_xfit + temp_b(j)*temp_y^j
      fit_diff = temp_xfit - old_xfit
      if (max(abs(fit_diff)) eq 0) then begin
         fit_converged = 1
      endif else begin
         good_ls = where(abs(temp_x-temp_xfit) le 3*xerr,n_good)
   ...
   b(*,i) = temp_b
   ; Plot residuals
   temp_xresid = temp_x - temp_xfit
   plot,temp_y,temp_xresid,/xstyle,/ystyle,psym=3,$
      title='y = '+strtrim(liney(i),2)+', x = '+strtrim(linex(i),2),$
      xtitle='y',ytitle='x residual',charsize=2
endfor
