Pro luci_flatfixer,flatlist,wavearr,slitarr,combflat,combflatvar,$
                      fixgain=fixgain,gain=gain,darklist=darklist,illum=illum

;+-------------------------------------------------------------------
;
; LUCI_FLATFIXER    2/2010
;
...
;       Revised 2/2010 Fuyan Bian @Steward Observatory
;---------------------------------------------------------------------
...
bigflat    = fltarr(2048,2048,nflat)
bigflatvar = fltarr(2048,2048,nflat)
...
exposed = where(slitgrid ge 550 and slitgrid le 1500)
notexposed = where(slitgrid lt 550 and slitgrid gt 1500)


   
   ;;; Correct for difference in blue vs. red gain (optional)
; set the value of the gain
...
;                       slitgrid ge 550 and slitgrid le 775)
      centerls = where(wavegrid ge 1000 and wavegrid le 1040 and $
                       slitgrid ge 600 and slitgrid le 1400)
      ...
      specls = where(slitgrid ge 1290 and slitgrid le 1310)
      ...
      tmp_ls = where(specwave ge 950 and specwave le 1050)
      ...
      for i=0,1 do begin
         ...
         if left then begin
            ...
            slithi = 1530
          endelse
        showls = where(wavegrid ge 300 and wavegrid le 500 and $
         ...
         if left then begin
            ...
            xedge_hi = xedge(3)
         endelse
         edgels = where(wavegrid ge 300 and wavegrid le 500 and $
...
;   dispmax=1.2*median(flatfix(650:750,450:550))
    dispmin = 0.8*median(flatfix(950:1050,950:1050))
    dispmax = 1.2*median(flatfix(950:1050,950:1050))
   ; Displayed region
   device,get_screen_size=ssz
   screen_xsz = ssz(0)
   screen_ysz = ssz(1)
   margin     = 50
   disp_xlo   = round(1024 - screen_xsz/2. + margin) > 0     
   disp_xhi   = round(1024 + screen_xsz/2. - margin) < 2047
   disp_ylo   = round(1024 - screen_ysz/2. + margin) > 0
   disp_yhi   = round(1024 + screen_ysz/2. - margin) < 2047
...
if (nflat eq 1) then begin
   ...
   medflat    = fltarr(2048,2048) - 99999.
   medflatvar = fltarr(2048,2048) + 10000.
