PRO CHAN, IPLANE, PLOT=plot, FREE=free, TITLE=title
;+
; NAME:
;       CHAN
; PURPOSE:
...
;	TITLE - Title to display at head of window (as in WINDOW function)
;
;
; COMMON BLOCKS:
...
;	Added TITLE keyword, G. D. Becker July 2002
;-
;
 On_Error,2
 ...
 if N_params() EQ 0 then begin 
        print, "Syntax:  CHAN, Iplane, [/FREE , /PLOT, TITLE=title]"
        return
 endif
...
; OPND is a 128-element array which is 1 wherever window is open
 ...
 IF keyword_set(TITLE) THEN BEGIN
    titleprint = 'IDL '+strtrim(iplane,2) + ' ' + title
 ENDIF ELSE BEGIN
    titleprint = 'IDL '+strtrim(iplane,2) + ' Plot Window'  
 ENDELSE

 IF keyword_set(plot) then begin
             window, iplane, xsize=512,ysize=410,title = titleprint
         return
 endif
