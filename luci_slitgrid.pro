Pro luci_slitgrid,exposurelist,slitgrid
...
;      Revised 2/18/2009 Fuyan Bian @Steward Observatory for LUCIFER spectroscopic data reduction.
;--------------------------------------------------------------------------

if (n_params() lt 2) then begin
 print,'CALLING SEQUENCE: '
 print,'   luci_slitgrid,exposurelist,slitgrid'
 return
endif
...
;   y0 = y0 + 250
   ...
   while not(fit_converged) do begin
      temp_a   = poly_fit(xtr(good_ls),ytr(good_ls),trace_order,$
                        yerror=yerr)
      yfit     = 0*ytr
      for j=0,trace_order do yfit = yfit + temp_a(j)*xtr^j
      fit_diff = yfit - old_yfit
      if (max(abs(fit_diff)) eq 0) then begin
         fit_converged = 1
      endif else begin
         good_ls = where(abs(ytr-yfit) le 3*yerr,n_good)
   ...
   ; Plot residuals
   yresid = ytr - yfit
   yresid_sdev = robust_sigma(yresid)
   chan,12
   plot,yresid,xtr,/xstyle,/ystyle,psym=3,title=temp_objname,$
      xtitle='y residual',ytitle='x',xrange=[-5*yresid_sdev,5*yresid_sdev]
endfor
